...
export class MockPddiktiAdapter implements PddiktiSyncAdapter {
  ...
  private generateMockData(): void {
    // Mahasiswa
    this.mahasiswaData = [
      { nim: 'MHS001', nama: 'Budi Santoso', prodiKode: 'TI', angkatan: 2023, status: 'aktif', email: 'budi@mhs.local', noHp: '081234567890' },
      { nim: 'MHS002', nama: 'Siti Rahayu', prodiKode: 'TI', angkatan: 2023, status: 'aktif', email: 'siti@mhs.local', noHp: '081234567891' },
      { nim: 'MHS003', nama: 'Ahmad Wijaya', prodiKode: 'SI', angkatan: 2022, status: 'aktif', email: 'ahmad@mhs.local', noHp: '081234567892' },
      { nim: 'MHS004', nama: 'Dewi Lestari', prodiKode: 'SI', angkatan: 2022, status: 'cuti', email: 'dewi@mhs.local', noHp: '081234567893' },
      { nim: 'MHS005', nama: 'Rudi Hartono', prodiKode: 'TI', angkatan: 2021, status: 'lulus', email: 'rudi@mhs.local', noHp: '081234567894' },
    ];
    
    // Dosen
    this.dosenData = [
      { nidn: '0012345601', nama: 'Dr. Andi Saputra', prodiKode: 'TI', jenisKelamin: 'L', statusKepengurusan: 'tetap', email: 'andi@dosen.local', noHp: '081234567800' },
      { nidn: '0012345602', nama: 'Prof. Siti Aminah', prodiKode: 'TI', jenisKelamin: 'P', statusKepengurusan: 'tetap', email: 'siti@dosen.local', noHp: '081234567801' },
      { nidn: '0012345603', nama: 'Ir. Budi Prasetyo', prodiKode: 'SI', jenisKelamin: 'L', statusKepengurusan: 'tidak_tetap', email: 'budi@dosen.local', noHp: '081234567802' },
    ];
    
    // Nilai
    this.nilaiData = [
      { nim: 'MHS001', kodeMk: 'IF101', semester: '20241', nilaiAngka: 85, nilaiHuruf: 'A', bobot: 4.00 },
      { nim: 'MHS001', kodeMk: 'IF102', semester: '20241', nilaiAngka: 78, nilaiHuruf: 'B+', bobot: 3.50 },
      { nim: 'MHS002', kodeMk: 'IF101', semester: '20241', nilaiAngka: 92, nilaiHuruf: 'A', bobot: 4.00 },
      { nim: 'MHS003', kodeMk: 'SI201', semester: '20241', nilaiAngka: 70, nilaiHuruf: 'B', bobot: 3.00 },
      { nim: 'MHS003', kodeMk: 'SI202', semester: '20241', nilaiAngka: 88, nilaiHuruf: 'A', bobot: 4.00 },
    ];
  }
  ...
  async syncMahasiswa(data: PddiktiMahasiswa[]): Promise<SyncResult<PddiktiMahasiswa>> {
    const result: SyncResult<PddiktiMahasiswa> = { total: data.length, created: 0, updated: 0, skipped: 0, errors: [] };
  ...
  async syncDosen(data: PddiktiDosen[]): Promise<SyncResult<PddiktiDosen>> {
    const result: SyncResult<PddiktiDosen> = { total: data.length, created: 0, updated: 0, skipped: 0, errors: [] };
  ...
  async syncNilai(data: PddiktiNilai[]): Promise<SyncResult<PddiktiNilai>> {
    const result: SyncResult<PddiktiNilai> = { total: data.length, created: 0, updated: 0, skipped: 0, errors: [] };
    
    for (const nilai of data) {
      try {
        const idx = this.nilaiData.findIndex(n => n.nim === nilai.nim && n.kodeMk === nilai.kodeMk && n.semester === nilai.semester);
        if (idx >= 0) {
