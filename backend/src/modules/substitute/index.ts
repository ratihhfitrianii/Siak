...
function requireLecturerOrAdmin(req: Request): { lecturerId: number | null; isAdmin: boolean } {
  const isAdmin = req.user!.roleCode === 'admin_akademik' || req.user!.roleCode === 'admin_sistem';
  if (isAdmin) return { lecturerId: null, isAdmin: true };
  if (!req.user?.lecturerId) {
    throw new AppError('FORBIDDEN', 'Akun bukan dosen aktif', 403);
  }
  return { lecturerId: req.user.lecturerId, isAdmin: false };
}
  ...
  try {
    ...
    for (const student of studentsRes.rows) {
      const title = action === 'created'
        ? 'Pengganti Mengajar Baru'
        : 'Pengganti Mengajar Dibatalkan';
  ...
  router.post(
    ...
    async (req: Request, res: Response, next: NextFunction) => {
      try {
        ...
        const subCheck = await pgPool.query(
          `SELECT 1 FROM lecturers WHERE id = $1 AND is_active`,
        ...
        const result = await pgPool.query(
          `INSERT INTO substitute_teaching
           ...
           RETURNING *`,
          [originalLecturerId, substituteLecturerId, classId, scheduleId, reason ?? null, req.user!.id]
        );
        ...
        await sendSubstituteNotification(classId, originalLecturerId, substituteLecturerId, scheduleId, reason, 'created');
    ...
    async (req: Request, res: Response, next: NextFunction) => {
      try {
        ...
        if (result.rows.length === 0) {
          return res.status(404).json({ success: false, error: 'Substitute teaching tidak ditemukan' });
        }
    ...
    async (req: Request, res: Response, next: NextFunction) => {
      try {
        ...
        if (existingRes.rows.length === 0) {
          return res.status(404).json({ success: false, error: 'Substitute teaching tidak ditemukan atau tidak berhak membatalkan' });
        }
