...
export async function generatePayroll(
  ...
  try {
    ...
    const baseSalary =
      lecturer.employment_type === 'tetap' ? config.baseSalaryTetap : config.baseSalaryKontrak;
...
export async function getPayrollDetail(payrollId: number): Promise<PayrollItem | null> {
  ...
  const breakdown = await getPayrollBreakdown(
    payroll.lecturer_id,
...
export async function batchGeneratePayroll(
  ...
  const lecturersRes = await pgPool.query(`SELECT id FROM lecturers WHERE is_active = true`);
