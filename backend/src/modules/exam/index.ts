...
export const createExamRouter = () => {
  ...
  router.get('/periods', authenticate, authorize('exam.period.manage'), async (req: Request, res: Response, next: NextFunction) => {
    try {
  ...
  router.post('/periods', authenticate, authorize('exam.period.manage'), async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      const result = await pgPool.query(
        ...
        [parsed.name, parsed.semester_id, parsed.start_date, parsed.end_date, parsed.is_active ?? true, parsed.can_input_questions ?? true]
      );
