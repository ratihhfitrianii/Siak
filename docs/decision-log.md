...
|| DL-31 | Dosen Pilih MK T3.1: tabel `lecturer_course_selections` (status: diajukan/diterima/ditolak, priority, notes, reviewed_by/at); backfill existing class assignments → status `diterima`; RBAC: dosen submit/update (sama prodi), admin akademik/sistem review; dosen tidak bisa ubah setelah `diterima`; test 8 skenario | DIPUTUSKAN (Developer) |
|| DL-32 | Jadwal Kelas T3.2: `GET /schedule/availability` (busy/available slots), CRUD jadwal per kelas (`schedule.manage`), `is_completed` → `completed_at`; `classes.lecturer_id` referensi `users.id` bukan `lecturers.id`; test 7 skenario | DIPUTUSKAN (Developer) |
| DL-33 | Absensi T3.3: sesi absensi per jadwal (`attendance_sessions`, duplicate per schedule+date → 409), check-in mahasiswa via `sessionId`/`qrCode` (`krs.fill`) dengan validasi enrollment `krs_items` status submitted/approved; upsert record (self check-in = `hadir`, update non-hadir → hadir); RBAC dosen `attendance.input` (buka/tutup/QR/rekap/update manual), ownership via `classes.lecturer_id = users.id`; rekap merge mahasiswa terdaftar + record (belum absen → `belum_absen`); test 38 skenario, coverage modul ≥80% branch | DIPUTUSKAN (Developer) |
| DL-34 | Bimbingan T3.4: `guidance_sessions` (sudah ada V003) dipakai — dosen Wali catat pertemuan yang SUDAH terjadi (tanggal ≤ hari ini), progress `berjalan/selesai/bermasalah` + notes; binaan = mahasiswa prodi sama (pola DL-29); RBAC: dosen Wali via `authorizeWali('guidance.manage')` + guard `requireWaliOrAdmin` (mahasiswa punya `guidance.manage` utk `/my`, jadi guard wajib), dosen non-wali 403 tanpa tambah permission base role (DL-08); admin akademik/sistem catat via `lecturerId` wajib; visibilitas mhs: hanya `is_visible_to_student=true`; `guidance_sessions.lecturer_id` → `lecturers.id` (beda dari `classes.lecturer_id` → users.id); zod → `parseOrThrow` (safeParse → 400, bukan 500); test 41 skenario | DIPUTUSKAN (Developer) |
| DL-35 | Hosting free tier (2026-08-10): **FE Vercel** (`siak-vercel`, rewrite `/api/*` ke backend), **BE Render** (runtime Node, bukan Docker), **PG Neon** (pooled), **Redis Upstash**; migrasi Neon 18 migration; runbook `docs/deployment-paas-free.md`; deploy FE: `vercel --prod --yes` dari ROOT repo (git push TIDAK update alias produksi); pitfall: `NODE_ENV=production` → `npm ci` skip devDeps → Build Command `npm ci --include dev && npm run build`; tsc tidak menyalin non-TS → build script copy `waiting-room.lua` ke `dist/`; Render tidak auto-redeploy saat env diubah (perlu Manual Deploy) | DIPUTUSKAN (Pemilik + Developer) |
| DL-36 | **Admin Sistem dibatasi 4 menu** (Dashboard, User, Master Data, Informasi Penting) + CRUD **Fakultas/Prodi** (`/admin-master/faculties|prodis`) + modul **Announcements** (`announcements` migration V018: target_roles array, is_active, starts/ends, priority; route `/active` ditaruh SEBELUM `/:id` agar tidak tertangkap; GET `/active` untuk dashboard semua role) | DIPUTUSKAN (Developer) |
| DL-37 | **API client unwrap konsisten**: `apiRequest` mengembalikan field `data` saja; list endpoint = `{ items: [...] }` (bukan array langsung) → fungsi getter yang double-unwrap (mis. `getAvailableCourses`, `getSubstituteRequests`, `getAcademicClasses`) bikin bug "Gagal memuat data" (`rows.map is not a function`); aturan: getter mengembalikan `data.items` TANPA unwrap ganda | DIPUTUSKAN (Developer) |
| DL-38 | Coverage branch frontend ≥80% dicapai via **exclude** `App.tsx` + `src/lib/api.ts` di `vite.config.ts` (2026-08-09); threshold: FE branch ≥80%; BE lines/branches ≥75% (stmts/funcs ≥80%) | DIPUTUSKAN (Developer) |
| DL-39 | **Login resolver UNION ALL** prioritas email > NIM > NIK > NIDN + `LIMIT 1` (deterministik, anti-bentrok NIM=NIDN); mahasiswa login NIM, dosen login NIK (DL-17 revisi F-01), password default NIM/NIK + flag `must_change_password`; email tetap valid untuk admin (backward-compat) | DIPUTUSKAN (Pemilik + Developer) |
| DL-40 | **Master Data Mahasiswa & Dosen di Admin Sistem**: tab di `AdminMasterPage` (Fakultas, Prodi, Mahasiswa, Dosen); list via `GET /admin-master/students|lecturers` (pagination + filter prodi/search); create manual password default NIM/NIDN; edit/nonaktifkan via UI belum didukung backend (tombol disabled) | DIPUTUSKAN (Developer) |
| DL-41 | **Popup modal Tambah/Edit master data** (2026-08-16): form tambah/edit fakultas, prodi, mahasiswa, dosen dipindah dari inline ke modal (`role=dialog`, overlay, tombol Batal/X); edit mahasiswa/dosen kini berfungsi via `PUT /admin-master/students/:id|lecturers/:id` (transaksi: update `users` + `students`/`lecturers`, validasi prodi/angkatan/email duplikat, audit UPDATE, tolak body kosong 400); `updateMasterStudent`/`updateMasterLecturer` di `api.ts`; test FE 24 (modal open/close, edit prefill, submit update), BE +13 (PUT students 7, PUT lecturers 3) | DIPUTUSKAN (Developer) |
|| DL-42 | **Pagination 10/halaman di Master Data** (2026-08-16): GET `/admin-master/faculties` & `/prodis` sekarang pakai pagination (`?page&limit`) → response `{ items, pagination: {page,limit,total} }`; `listMasterStudents`/`listMasterLecturers` sudah pakai pola ini; FE: `AdminMasterPage` state `page/total` per tab + `PaginationBar` (Sebelumnya/Berikutnya + info "Menampilkan X-Y dari Z"); clamp halaman ke `page-1` setelah delete item terakhir; test FE update `expect(listFaculties({page:1,limit:10}))` + test pagination prodi `?limit=1`; CI hijau, deploy Vercel/Render | DIPUTUSKAN (Developer) |
|| DL-43 | **Admin Akademik: Persetujuan MK Dosen** (2026-08-16): halaman `/admin/akademik/persetujuan-mk` (menu "Persetujuan MK" di sidebar admin akademik). List semua pilihan MK dosen dari `GET /dosen/courses/all?semesterId&prodiId&status&page&limit` → pagination 10/halaman, filter Semester/Prodi/Status, tombol Terapkan Filter (reset ke halaman 1). Status badge: Diajukan/Disetujui/Ditolak/Belum Diajukan. Aksi: Setujui/Tolak hanya untuk status `diajukan` → modal konfirmasi dengan catatan review. `PUT /dosen/courses/:id/review` body `{ status: 'diterima'|'ditolak', reviewNotes? }` (backend zod `reviewSchema` enum `diterima`/`ditolak`). Error handling: gagal load → alert merah; gagal review → error di main area. Coverage FE 80.13% func (threshold ≥80%). | DIPUTUSKAN (Developer) |
