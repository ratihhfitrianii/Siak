...
|| **Iterasi 3 — Dosen Mengajar** | T3.1–T3.8 | Pilih MK, jadwal, absensi, bimbingan, substitute, nilai detail | ~20 hari | 🟢 **T3.1 SELESAI** (Dosen Pilih MK + migration + 8 test); 🟢 **T3.2 SELESAI** (Jadwal Kelas + Ketersediaan + 7 test); 🟢 **T3.3 SELESAI** (Absensi + QR check-in + 38 test, coverage modul ≥80% branch); 🟢 **T3.4 SELESAI** (Bimbingan Wali + 41 test, coverage modul 89.36% branch); ⬜ T3.5–T3.8 menunggu |
