...
export const StudentExamPage: React.FC = () => {
  ...
  const fetchExams = async () => {
    try {
      setLoading(true);
      const res = await apiRequest('/exam/student/my-exams');
      setExams(res.data || []);
    } catch (err: unknown) {
        ...
        <table className="w-full text-left border-collapse">
          ...
          <tbody>
            {loading ? (
              ...
              <tr><td colSpan={5} className="p-4 text-center">Tidak ada jadwal ujian aktif.</td></tr>
            ) : (
              exams.map((ex) => (
                <tr key={String(ex.id)} className="border-b">
                  ...
                  <td className="p-3">{String(ex.start_time)} - {String(ex.end_time)}</td>
                  <td className="p-3">{String(ex.room || '-')}</td>
                  <td className="p-3">
                    <a href="https://pessay.ai" target="_blank" rel="noreferrer" className="bg-green-600 text-white px-3 py-1 rounded text-sm">
                      Mulai Ujian (Pessay)
                    </a>
                  </td>
                </tr>
