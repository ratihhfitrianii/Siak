...
export function DosenBimbinganBerjalan() {
  const { user } = useAuth();
  ...
  const loadProposals = useCallback(async () => {
    try {
      ...
      // Filter: dosen is supervisor AND status is active (dalam_bimbingan or later)
      const filtered = data.filter(
        (p) => p.supervisors?.some((s) => s.id === user?.id) && ACTIVE_STATUSES.includes(p.status),
      );
      setProposals(filtered);
    } catch {
      setError('Gagal memuat daftar bimbingan berjalan');
    } finally {
      setIsLoading(false);
    }
  }, [user?.id]);
  ...
  if (isLoading) {
    return (
      <div className="flex min-h-[50vh] items-center justify-center">
        <Spinner label="Memuat bimbingan berjalan..." />
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6">
      <header className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-semibold text-slate-900">Bimbingan Berjalan</h1>
          <p className="text-slate-500 mt-1">
            Daftar mahasiswa yang sedang dalam proses bimbingan skripsi dengan Anda
          </p>
        </div>
      </header>
      ...
      {proposals.length === 0 ? (
        <div className="rounded-xl border border-slate-200 bg-white p-12 text-center">
          ...
          <h3 className="mt-4 text-lg font-medium text-slate-900">Belum ada bimbingan berjalan</h3>
          <p className="mt-1 text-slate-500">
            Mahasiswa yang Anda bimbing akan muncul di sini setelah proposal disetujui.
          </p>
        </div>
      ) : (
